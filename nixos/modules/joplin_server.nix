...
let
  httpPort = 8014;
  dataDir = "/data/apps/joplin_server";
  joplinUrl = "https://joplin.vnunes.win";
in {
  imports = [ ./podman.nix ];
  # ── PostgreSQL ──────────────────────────────────────────────────────────────
  services.postgresql = {
  ...
  virtualisation.oci-containers.containers.joplin_server = {
    ...
    environment = {
      APP_BASE_URL = joplinUrl;
      API_BASE_URL = joplinUrl;
      DB_CLIENT = "pg";
      POSTGRES_PORT = "5432";
