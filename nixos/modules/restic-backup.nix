...
in {
  options.myServices.resticBackup = {
    ...
    rcloneB2 = {
      ...
      timerConfig = lib.mkOption {
        type    = lib.types.attrs;
        default = { OnCalendar = "Sat *-*-* 04:00:00"; Persistent = true; };
      };
    };
  };
  ...
  config = lib.mkIf cfg.enable {
    services.postgresqlBackup = lib.mkIf pgCfg.enable {
      location    = pgCfg.location;
      startAt     = pgCfg.startAt;
      compression = pgCfg.compression;
      databases   = pgCfg.databases;
    };
    ...
    systemd.timers."${hostName}-rclone-b2" = lib.mkIf b2Cfg.enable {
      wantedBy    = [ "timers.target" ];
      timerConfig = b2Cfg.timerConfig;
    };
    systemd.services."${hostName}-rclone-b2" = lib.mkIf b2Cfg.enable {
